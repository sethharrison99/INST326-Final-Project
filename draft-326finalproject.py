...
class Restaurant:
    ...
    def add_to_order(self, item, order, options):
        ...
        """
        if item in options:
            customized_item = self.customize_item(item, options)
...
def logIn(self, file):
            opener = input("Would you like to login or create an account? 1 for login, 2 for create an account ")
            opener = int(opener)
            if opener == 1:
                ...
                for line in fileread.readlines():
                    ...
                    if(uspw[0] == user_name and uspw[1] == pass_word):
                ...
                user_name = input("Enter your desired username: (Must be 8 characters or longer, and must have at least one letter, with no special characters)")
                while (len(user_name) <5 or user_name.isalnum()== False):
                    user_name = input("User Name invalid; Enter your desired username: (Must be 8 characters or longer, and must have at least one letter)")
                                
                print("Valid Username!")
                pass_word = input("Enter your password: (Must be 10 characters or longer)")
                while (len(pass_word)<10):
                    pass_word = input("Password invalid; Enter your desired password: (Must be 10 characters or longer)")
...
"""
 if __name__ == "__main__":
    f= open("users.txt","w")
    f.write("JohnDoe bigguy1990 \n")
    f.write("JimmyNeutron richman1800 \n")
    f.write("TimmyTurner bigmoneyballa02 \n")
    f.write("MickeyMouse kaydashplash \n")
    ...
    print(r.take_order(options))
    
    uss = r.logIn("users.txt")
    if uss == False:
        while uss == False:
            uss = r.logIn("users.txt")
